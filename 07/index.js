  ...
  states: {
    ...
    checkingAuth: {
      on: {
        '': [
          {
            cond: (context, event) => !!context.user,
            target: 'idle',
          },
          {
            target: 'unauthorized'
...
//First var to provide a user: HARDCODING
// const service = interpret(dragDropMachine.withContext({
//   ...dragDropMachine.initialState.context,
//   user: { name: 'Loly' }
// }));

//Second var: we changed dragDropMachine ---> createDragDropMachine to a func that takes user
const service = interpret(
  createdragDropMachine({ name: 'Loly' })
);
